...
def export_show(program_id, plot, icon):
    ...
    for show in json_source:
        ...
        plot = 'null'
        icon = 'null'
        # Create folder called "Library" to save .strm files
        path = xbmc.translatePath(os.path.join(ADDON.getSetting(id='library_folder'), 'Library')
                                  + '/' + 'TV Shows' + '/')
        ...
        if xbmcvfs.exists(path):
            ...
            file_content = 'plugin://plugin.video.solaris.tv/?mode=900&url=%s&plot=%s&program_id=%s&series_id=%s' \
...
def get_json(url, life=timedelta(minutes=1)):
    headers = {
        'Accept': '*/*',
        'reqPayload': ADDON.getSetting(id='EPGreqPayloadDir'),
        'User-Agent': UA_ANDROID_TV,
        'Accept-Encoding': 'gzip, deflate, br',
...
def add_show(name, mode, icon, fanart, info, show_info):
    ...
    context_items = [
        ('Add To Favorites Channels',
         'RunPlugin(plugin://plugin.video.solaris.tv/?mode=1001&fav_type=channel%s)' % show_values),
        ('Remove From Favorites Channels',
         'RunPlugin(plugin://plugin.video.solaris.tv/?mode=1002&fav_type=channel%s)' % show_values),
        ('Add To My DVR',
         'RunPlugin(plugin://plugin.video.solaris.tv/?mode=1001&fav_type=show%s)' % show_values),
        ('Remove From My DVR',
         'RunPlugin(plugin://plugin.video.solaris.tv/?mode=1002&fav_type=show%s)' % show_values),
        ('Add To Library',
         'RunPlugin(plugin://plugin.video.solaris.tv/?mode=850%s)' % show_values)
    ]
